...
exports.GET= async (req,res)=>{
        ...
        try {
            ...
            await page.goto(`${process.env.CORRELATIVIDADCURSADO}${legajo}-${token}`);

            try {
                await page.waitForSelector('.tituloTabla');

                const correlatividadCursado= await page.evaluate( ()=> {
                ...
                if(correlatividadCursado){
                res.json({msg:'OK',res:correlatividadCursado});
                }
            }
            catch(error) {
                    try {
