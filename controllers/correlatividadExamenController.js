...
exports.GET= async (req,res)=>{
        ...
        try {
            ...
            await page.goto(`${process.env.CORRELATIVIDADEXAMEN}${legajo}-${token}`);

            try {
                await page.waitForSelector('.tituloTabla');

                const correlatividadExamen= await page.evaluate( ()=> {
                ...
                if(correlatividadExamen){
                res.json({msg:'OK',res:correlatividadExamen});
                }
            }
            catch(error) {
                    try {
